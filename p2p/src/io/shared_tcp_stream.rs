...
impl SharedTcpStream {
	...
	fn new(stream: TcpStream) -> Self {
		SharedTcpStream { io: Arc::new(Mutex::new(stream)) }
	}

	pub async fn shutdown(&self) {
		// error is irrelevant here, the connection is dropped anyway
		let _ = self.io.lock().await.shutdown(Shutdown::Both);
	}
...
impl SharedTcpStream {
	...
	pub fn new(bytes: Bytes) -> Self {
		SharedTcpStream { read: Arc::new(Mutex::new(Cursor::new(bytes.into()))), write: Arc::new(Mutex::new(Bytes::default())) }
	}
	...
	pub async fn read_exact(&self,  mut buf: &mut [u8]) -> Result<usize, io::Error> {
