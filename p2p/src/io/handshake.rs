...
pub async fn handshake(a: &SharedTcpStream, magic: Magic, version: Version, min_version: u32) -> MessageResult<HandshakeResult> {
...
pub async fn accept_handshake(a: &SharedTcpStream, magic: Magic, version: Version, min_version: u32) -> MessageResult<HandshakeResult> {
	let peer_version: Version = read_message(a, magic, 0).await?;
...
mod tests {
	...
	async fn test_self_handshake() {
		...
		let hs = handshake(&shared_stream, magic, local_version, 0).await;
		assert_eq!(hs.unwrap_err(), expected);
	}
