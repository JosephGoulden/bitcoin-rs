...
use crate::net::{accept_connection, connect, Connections, ConnectionCounter, Channel};
use crate::util::{NodeTable, Node};
use crate::{LocalSyncNodeRef, Config, NodeTableError, NetConfig, Direction, PeerId, OutboundSyncConnectionRef, InboundSyncConnectionRef};
use crate::session::{NormalSessionFactory, SessionFactory, SeednodeSessionFactory};
...
impl Context {
	...
	pub async fn accept_connection_future(
		context: Arc<Context>,
		stream: TcpStream,
		...
		config: NetConfig,
			...
			match accept_connection(stream, &config, socket).await {
				Ok(DeadlineStatus::Meet(connection)) => {
					// successful handshake
	...
	pub async fn on_message(context: Arc<Context>, channel: Arc<Channel>) -> MessageResult<()> {
		...
		T: Payload,
	{
		match context.connections.channel(peer) {
			Some(channel) => {
				...
				let message =
					Message::with_flags(info.magic, info.version, &payload, serialization_flags).expect("failed to create outgoing message");
	...
	pub async fn send<T>(_context: Arc<Context>, channel: Arc<Channel>, message: T)
	where
		T: AsRef<[u8]> + Send + 'static,
	{
		//trace!("Sending {} message to {}", T::command(), channel.peer_info().address);
		let result = channel.write_message(message).await;
		match result {
			Ok(_) => {
				// successful send
...
impl P2P {
	...
	pub async fn run(self) {
		...
		let resolver = Arc::new(TokioAsyncResolver::from_system_conf(tokio::runtime::Handle::current()).await.expect("could not instantiate dns resolver"));
